//! Account module -- account pool, HTTP client, PoW solver
//!
...
/// Unified entry point for the account module
///
/// Encapsulates AccountPool, DsClient, and PowSolver, providing a unified interface to upper layers (chat, DsCore).
pub struct Accounts {
...
impl Accounts {
    /// Create and initialize the account module
    ///
    ...
    /// Acquire an account (wait up to timeout_ms milliseconds)
    ...
    /// Acquire an account (return immediately)
    ...
    /// Get HTTP client (for use when constructing GuardedStream)
    ...
    /// Upload a file (includes PoW computation and polling)
                ...
                match file.status.as_str() {
                    "SUCCESS" => {
                        log::debug!(
                            ...
                            "req={} file upload successful: file_id={}, tokens={:?}, name={}",
    ...
    /// Compute the PoW header for the specified target_path
