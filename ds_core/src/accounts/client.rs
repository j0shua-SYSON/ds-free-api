...
//!
//! Stateless: no caching, no retries, no session state.
//! Each method corresponds to one REST endpoint (see docs/ds-api-reference.md).
//! Streaming methods (completion/edit_message) return raw byte streams; SSE parsing is done by the upper layer.
//!
//! Contains only minimal business logic: HTTP error code and business error code parsing (into_result).
...
impl<T: serde::de::DeserializeOwned> Envelope<T> {
    fn into_result(self) -> Result<T, ClientError> {
        ...
        data.biz_data.map_or_else(
            || {
                // Allow biz_data to be null; attempt to construct T from null (succeeds only when T is Option)
...
impl DsClient {
    ...
    /// Cancel an ongoing streaming output; PoW is not required
    ...
    /// Upload a file and return its metadata (id, status, etc.)
    ...
    /// Query file status and return the file list (including status: PENDING/SUCCESS/FAILED)
