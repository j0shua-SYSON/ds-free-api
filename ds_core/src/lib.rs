//! DeepSeek core module -- adaptation layer from OpenAI API to DeepSeek
//!
//! Exposes a minimal public interface: DsCore, CoreError, ChatRequest, DsCoreConfig, AccountConfig
...
pub enum CoreError {
    ...
    /// Provider error: network, business error, token expiry, etc.
    #[error("provider: {0}")]
...
impl DsCore {
    ...
    /// Initiate a chat request; returns SSE byte stream with account identifier
    ///
    /// Account is automatically released when the stream ends or is dropped
    ...
    /// Graceful shutdown: clean up sessions for all accounts
