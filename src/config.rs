//! Configuration loading module -- unified config entry point
//!
//! Supports the `-c <path>` CLI argument; defaults are defined in the functions below.
//! Commented-out entries in config.toml use the code defaults.
...
pub struct Config {
    ...
    /// Admin configuration (bcrypt password hash, JWT secret, etc. -- managed via the admin panel)
    #[serde(default)]
    pub admin: AdminConfig,
    /// API key list (managed via the admin panel)
    #[serde(default)]
...
pub struct DsCoreSection {
    /// Account pool (required; may be empty -- accounts can be added via the admin panel after startup)
    #[serde(default)]
    pub accounts: Vec<Account>,
    /// API base URL
    #[serde(default = "default_api_base")]
    pub api_base: String,
    /// Full URL of the WASM file (required for PoW computation; version number may change)
    #[serde(default = "default_wasm_url")]
    pub wasm_url: String,
    /// User-Agent request header
    #[serde(default = "default_user_agent")]
    pub user_agent: String,
    /// X-Client-Version request header (used for expert model and similar features)
    ...
    /// List of supported model types; each type is automatically mapped to the OpenAI model_id: deepseek-<type>
    #[serde(default = "default_model_types")]
    pub model_types: Vec<String>,
    /// Input token limit per model type (index-aligned with model_types)
    #[serde(default = "default_max_input_tokens")]
    pub max_input_tokens: Vec<u32>,
    /// Output token limit per model type (index-aligned with model_types)
    #[serde(default = "default_max_output_tokens")]
    pub max_output_tokens: Vec<u32>,
    /// Input character limit per model type per request (index-aligned with model_types)
    #[serde(default = "default_input_character_limits")]
    pub input_character_limits: Vec<u32>,
    /// Model aliases: index-aligned with model_types; no aliases by default
    #[serde(default)]
...
pub struct AdminConfig {
    ...
    /// Timestamp of the most recent JWT issuance (used to revoke old tokens)
    #[serde(default)]
    pub jwt_issued_at: u64,
    /// Password change: old password in plaintext (only received by PUT, never persisted to config.toml)
    #[serde(default, skip_serializing)]
    pub old_password: String,
    /// Password change: new password in plaintext (only received by PUT, never persisted to config.toml)
    #[serde(default, skip_serializing)]
...
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct Account {
    /// Email address (mutually exclusive with mobile)
    pub email: String,
    /// Mobile number (mutually exclusive with email)
    pub mobile: String,
    /// Country code (used together with mobile, e.g. "+86")
...
pub struct ToolCallTagConfig {
    /// Extra start tags (built-in `<|tool▁calls▁begin|>` + fuzzy matching are always active; only add variants with a completely different format here)
    #[serde(default = "default_tool_call_starts")]
    pub extra_starts: Vec<String>,
    /// Extra end tags (built-in `<|tool▁calls▁end|>` + fuzzy matching are always active; only add variants with a completely different format here)
...
pub struct ServerConfig {
    ...
    /// Listen port
    pub port: u16,
    /// Allowed CORS origin list; default: ["http://localhost:22217"]
...
impl Config {
    ...
    /// Deduplicate accounts by email (preferred) or mobile, keeping the first occurrence
