//! OpenAI chunk generator -- maps StreamEvents to ChatCompletionsResponseChunk
            ...
            match this.inner.as_mut().poll_next(cx) {
                Poll::Ready(Some(Ok(event))) => match event {
                    ...
                    StreamEvent::ThinkStart => {
                        // The converter does not need special handling for ThinkStart
                        continue;
                    }
                ...
                Poll::Ready(None) => {
                    if !*this.finished {
                        warn!(target: "adapter", "converter stream ended prematurely: model={}", this.model);
                    }
