//! HTTP server layer -- thin routing shell that exposes OpenAIAdapter and AnthropicCompat as HTTP endpoints
//!
//! This module wraps the adapter/compat layer into an axum HTTP service.
...
pub async fn run(config: Config, config_path: PathBuf) -> anyhow::Result<()> {
    ...
    log::info!(target: "http::server", "anthropic-compatible base_url: http://{}/anthropic", addr);
...
fn build_router(state: AppState, cors_origins: Vec<String>) -> Router {
    ...
    // Static file service: /admin -> web/dist/
    // Prefer reading from the filesystem (development mode), falling back to compile-time embedded assets (release binary)
    ...
    let router = if web_dist.exists() {
        ...
        // Compile-time embedded: fallback mode, no specific routes registered, no conflict risk
...
/// Compile-time embedding of the web/dist/ directory; the release binary can serve the admin panel without any additional files
#[derive(rust_embed::Embed)]
#[folder = "web/dist/"]
struct WebAssets;

/// Embedded asset fallback: only handles /admin and /admin/* paths; all other paths return 404
...
/// API key authentication middleware (validates the Bearer token against api_keys.json)
...
/// JWT authentication middleware (admin panel routes)
...
async fn shutdown_signal() {
    ...
    log::info!(target: "http::server", "received shutdown signal, starting graceful shutdown");
}
