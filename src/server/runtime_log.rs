//! Runtime logging -- custom log::Log implementation with three-way output + file rotation
//!
//! Three outputs: stderr (terminal-visible) + in-memory ring buffer (queryable via API) + file (persistent)
//! File rotation: 10 MB per file, 3 history files retained, total cap ~40 MB
...
/// Number of history log files to retain
...
pub struct DualLogger {
    ...
    /// Current log file (std::sync::Mutex used for non-blocking writes on the log path)
...
/// Return the ANSI color code for the given log level (stderr only)
...
/// Wraps Arc<DualLogger> to implement Log (required because set_boxed_logger expects Box<dyn Log>)
...
/// Query runtime logs (paginated, reverse-chronological order)
