...
/// Data structure for managing stats.json
...
impl StoreManager {
            ...
            match fs::read_to_string(&stats_path) {
                Ok(content) if !content.trim().is_empty() => {
                    match serde_json::from_str::<StatsStore>(&content) {
                        Ok(s) => {
                            info!(target: "store", "stats.json loaded");
                            s
                        }
                        Err(e) => {
                            warn!(target: "store", "stats.json parse failed: {}, using zero values", e);
                            StatsStore::default()
                        }
                    }
                }
                ...
                Err(e) => {
                    warn!(target: "store", "stats.json read failed: {}, using zero values", e);
                    StatsStore::default()
                }
            }
        } else {
            info!(target: "store", "stats.json not found, using zero values");
            StatsStore::default()
        };
    ...
    /// Check whether a password has been set
    ...
    /// Get the most recent JWT issuance timestamp (used to revoke old tokens)
...
/// Atomically write a JSON file: write to .tmp first, then rename
...
// hex encoding helper (avoids extra dependencies)
mod hex {
