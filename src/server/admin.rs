//! Admin API route handlers -- login/password setup, account pool status, request stats, model list, config view
...
/// POST /admin/api/setup -- initial password setup
pub(crate) async fn admin_setup(
    ...
    match super::auth::setup_admin(&state.store, &state.login_limiter, &req.password).await {
        ...
        Err(msg) => {
            ...
            } else if msg.contains("at least 6") {
                StatusCode::BAD_REQUEST
            } else {
    ...
    match super::auth::login_admin(&state.store, &state.login_limiter, &req.password).await {
        ...
        Err(msg) => {
            let status = if msg.contains("too many") {
                StatusCode::TOO_MANY_REQUESTS
            } else if msg.contains("no password set") {
                StatusCode::FORBIDDEN
            } else {
...
pub(crate) async fn admin_put_config(
    ...
    // API keys match by `id` (stable identifier), falling back to description for old-format migration.
    {
        ...
        // Admin config: empty password_hash/jwt_secret retain existing values (the frontend does not return these fields)
        ...
        if !new_config.admin.old_password.is_empty() || !new_config.admin.new_password.is_empty() {
            if new_config.admin.old_password.is_empty() || new_config.admin.new_password.is_empty()
            {
                return error_response(
                    StatusCode::BAD_REQUEST,
                    "password change requires both old_password and new_password",
                );
            }
...
/// GET /admin/api/logs -- fetch recent request logs
...
/// GET /admin/api/runtime-logs -- paginated query of runtime logs
