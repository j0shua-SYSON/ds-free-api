...
/// Statistics counters for a single model
...
/// Statistics counters for a single API key
pub struct KeyUsage {
    pub prompt_tokens: AtomicU64,
    pub completion_tokens: AtomicU64,
    pub requests: AtomicU64,
...
pub struct Stats {
    ...
    /// Value of total_requests at the last persistence
...
impl Stats {
    /// Create Stats, optionally restoring from persisted data (including per-model/key stats and request logs)
    ...
    /// Record token usage (including per-model and per-API-key breakdown)
    ...
    /// Check whether persistence is needed
    ...
    /// Immediately persist current statistics (including per-model/key breakdown and request logs)
    ...
    /// Generate a per-API-key statistics snapshot
    pub fn key_stats_snapshot(&self) -> HashMap<String, KeyUsageSnapshot> {
        self.key_stats
            .iter()
            .map(|r| {
                // Mask: show only the first 8 characters
                let masked = if r.key().len() > 8 {
                    format!("{}***", &r.key()[..8])
                } else {
                    "***".to_string()
...
/// If neither mark_success nor mark_failure is called, Drop records the request as a failure
...
impl RequestTimer {
    /// Mark the request as successful and record statistics
    ...
    /// Mark the request as failed and record statistics
