...
//!
//! Pure functions: do not accept JSON bytes, performs direct field mapping from struct to struct.
...
/// Map Anthropic MessagesRequest directly to a ChatCompletionsRequest struct
pub(crate) fn into_chat_completions(req: MessagesRequest) -> ChatCompletionsRequest {
    // messages: prepend system message + convert messages
...
/// Map assistant content blocks to OpenAI messages
...
/// Map user content blocks to OpenAI messages
fn user_blocks_to_messages(blocks: &[ContentBlock]) -> Vec<Message> {
        ...
        match block {
            ...
            ContentBlock::Document { source, title } => match source {
                ...
                ImageSource::Url { url } => {
                    ...
                    // format_part outputs [please visit this link: {url}]
    ...
    // merge text + images + files into a single user message
    if !text_parts.is_empty() || !image_parts.is_empty() || !file_parts.is_empty() {
        if image_parts.is_empty() && file_parts.is_empty() {
            ...
            // contains images: use the parts array
...
mod tests {
    ...
    fn image_source_mapped() {
        // both base64 and url image sources are mapped to image_url content parts
        let cases = [
            (
